...
// (see quappe-service/src/lib/server/identity.ts). The client learns its own
// id by calling GET /api/me once at boot and caches it in localStorage for
// synchronous access — but the server IGNORES any user_id/author_id fields
// in request bodies and trusts only the cookie.
//
...
import { bumpUserIdTick } from './user-tick.svelte';
...
export function getUserId(): string {
	...
	// No cookie roundtrip has happened yet AND no legacy value. Return an
	// empty string so `author_id === userId` comparisons harmlessly miss
	// until bootstrapUserId() populates the real value; do NOT mint a random
	// throwaway id here — that used to shadow the real cookie id in reactive
	// filters (see "Meine Thesen" bug) and could persist to localStorage.
	return '';
}
